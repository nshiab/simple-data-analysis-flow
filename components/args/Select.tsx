...
export default function Select({ id, i, method, d, args }: { id: string, i: number, method: string, d: Arg, args: NodeDataArgs }) {
    ...
    useEffect(() => {
        ...
        const t = <select id={argId} onChange={() => updateNodeArgs(id)} value={args[d.name]}>
            {d.options ? d.options.map((opt, index) => <option key={`${id}-${method}-option-${index}`}>{opt}</option>
            ) : null}
        </select>
