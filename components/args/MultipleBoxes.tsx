...
export default function MultipleKeys({ id, method, d, i, args }: { id: string, method: string, d: Arg, i: number, args: NodeDataArgs }) {
    ...
    useEffect(() => {
        ...
        const t = <div style={{ display: "flex", flexWrap: "wrap" }}>
            {d.options ? d.options.map((opt, index) => <div style={{ display: "flex", alignItems: "center", border: "1px solid grey", borderRadius: 5, padding: "5px 5px", margin: 3 }} key={`${id}-${method}-arg${i}-multipleBoxes${index}`}>
                <div>{opt}</div>
        ...
        setType(t)
    }, [id, method, d, i, args, generateArgId, updateNodeArgs])
