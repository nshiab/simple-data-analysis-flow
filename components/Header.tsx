...
export default function Header({ flowInstance, setHome, name, setName }) {
    ...
    return <div style={{ backgroundColor: "white", paddingTop: 5, paddingBottom: 5 }}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
            <HomeIcon style={{ fontSize: 20, cursor: "pointer" }} onClick={() => setHome(true)} />
            <div style={{ display: "flex" }}>
                {!editMode ? <div style={{ fontWeight: "bold", fontSize: 16 }}>{name}</div> : null}
                {editMode ? <input ref={inputRef} style={{ fontWeight: "bold", width: 400, textAlign: "center", fontSize: 16 }} defaultValue={name} onKeyUp={(evt) => {
                    if (evt.key === "Enter") {
                        setName(evt.target.value)
                        setEditMode(false)
                    }
                }}></input> : null}
                <EditIcon style={{ fill: editMode ? "black" : "grey", height: 18, cursor: "pointer" }} onClick={() => {
                    if (!editMode) {
                        setEditMode(true)
                    } else {
                        setEditMode(false)
            ...
            <DownloadIcon style={{ fill: editMode ? "grey" : "black", height: 18, cursor: "pointer" }} onClick={() => {
                if (!editMode) { onSave() }
            }} />
        </div>
    </div>
