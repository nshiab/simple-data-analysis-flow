...
export default function NewSimpleData({ id, data }: { id: string, data: NodeData }) {
    ...
    return <div style={{ backgroundColor: "white", border: "1px solid black", borderRadius: 5, padding: 10 }}>
        ...
        {data.openManualData ?
            <div style={{ display: "flex", flexDirection: "column", marginTop: 10 }}>
                <textarea rows={10} ref={inputRef} style={{ width: 200 }} placeholder='Paste a JSON array of objects' onChange={() => {
                    try {
                        const value = inputRef && inputRef.current ? inputRef.current.value : null
                        updateNodeSimpleData(id, null, null, { manualData: value === "" || value === null ? [] : JSON.parse(value), manualDataString: value })
                    } catch (error) {
                        //@ts-ignore
                        updateNodeSimpleData(id, null, error.message, {})
                        setSucces(false)
