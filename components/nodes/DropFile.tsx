        ...
        try {
            ...
            if (data.file && data.file.content && argsTest) {
                ...
                if (data.file.type === "text/csv") {
                    parsedContent = data.args.autoType
                        ? csvParse(data.file.content, autoType)
                        : csvParse(data.file.content)
                } else if (data.file.type === "application/json") {
                    parsedContent = JSON.parse(data.file.content)
                }

                data.args.firstItem =
                    data.args.firstItem === "" ? undefined : data.args.firstItem
                data.args.lastItem =
                    data.args.lastItem === "" ? undefined : data.args.lastItem
    ...
    return (
        ...
        >
            ...
            <div
                ...
                onDrop={async (evt) => {
                    ...
                    try {
                        if (items.length > 1) {
                            throw new Error(
                                "Multiple files dropped. You can only drop one."
                            )
                        }

                        if (
                            !["text/csv", "application/json"].includes(
                                items[0].type
                            )
                        ) {
                            throw new Error(
                        ...
                        if (f) {
                            ...
                            updateNodeSimpleData(id, data.simpleData, null, {
                                file: {
                                    content: fileContent,
                                    name: f?.name,
                                    type: f?.type,
            ...
            >
                ...
                >
                    Drop a CSV or JSON file (array of objects)
                </div>
            </div>
            {data.file ? (
                <div>
            ...
            {data.errorMessage ? (
                <div style={{ maxWidth: "100%", color: "red", marginTop: 10 }}>
                    {data.errorMessage}
                </div>
            ) : null}
            <Handle
