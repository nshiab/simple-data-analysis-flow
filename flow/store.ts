...
type RFState = {
    ...
    testNodeArgs: (method: string, args: NodeDataArgs) => boolean
...
const useStore = create<RFState>((set, get) => ({
    logs: false,
    startNodeId: 0,
    setStartNodeId: (number) => {
        set({ startNodeId: number })
    },
    nodes: [],
    edges: [],
    setNodes: (nodes) => {
    ...
    onConnect: (connection) => {
        ...
        const sourceSimpleData = nodes.find((d) => d.id === connection.source)
            ?.data.simpleData
        ...
        const edgesToRemove = edges.filter((d) => {
            const conditions =
                ...
                (d.target !== newEdge.target ||
                    d.targetHandle !== newEdge.targetHandle)
            return !conditions
        })
        ...
        if (method === "newSimpleData") {
            newNode = {
                ...
                position: {
                    ...
                    y:
                        lastNode && lastNode.height
                            ? lastNode.position.y + lastNode.height + 20
                            : 0,
                },
            }
    ...
    testNodeArgs: (method, args) => {
        const methods = get().methods

        let argsTest = false

        if (
            methods[method].arguments.filter((d) => d.optional === false)
                .length === 0
        ) {
            argsTest = true
        } else {
            for (let arg of methods[method].arguments.filter(
                (d) => d.optional === false
            )) {
                if (args[arg.name]) {
                    argsTest = true
    ...
    updateNodeArgs: (id) => {
        ...
        //@ts-ignore
        const { method, sourceSimpleDataB } = nodes.find(
            (d) => d.id === id
        ).data
        ...
        for (let i = 0; i < methods[method].arguments.length; i++) {
            if (
                ["text", "number", "keys", "select"].includes(
                    methods[method].arguments[i].type
                )
            ) {
                const el: HTMLInputElement | null = document.querySelector(
                    `#${get().generateArgId(id, i, method)}`
                )
                ...
                if (el) {
                    ...
                    if (methods[method].arguments[i].jsOption) {
                        const elOption: HTMLInputElement | null =
                            document.querySelector(
                                `#${get().generateArgId(id, i, method)}JS`
                            )
                    ...
                    if (methods[method].arguments[i].type === "number") {
                        ...
                        args[methods[method].arguments[i].name] = isNaN(
                            valNumber
                        )
                            ? undefined
                            : valNumber
                    } else {
                        args[methods[method].arguments[i].name] =
                            val === "" ? undefined : val
                    }
                }
                ...
                ["multipleKeys", "multipleBoxes"].includes(
                    methods[method].arguments[i].type
                )
            ) {
    ...
    remainingItemsShowTable: (data) => {
        ...
        const nbItemsInTable = data.args.nbItemsInTable
            ? data.args.nbItemsInTable
            : 5
